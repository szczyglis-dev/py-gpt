...
Allows HTML/JS code to be rendered in HTML Canvas (a built-in Chromium-based renderer). To use it, ask the model to render HTML/JS in the built-in HTML Canvas. The tool is integrated with the ``Python code interpreter`` plugin, and its application-managed Canvas HTML file is stored under ``%workdir%/tmp``.
