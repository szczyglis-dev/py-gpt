...
Use **New** to create a user-defined agent profile. Custom profiles are stored in ``config.json`` under ``agent.v2.custom_agents`` and are identified by UUID. Each custom profile has an editable name and its own complete system prompt. Custom profiles use the **Orchestrator** runtime/tool surface, which means their prompt can define a complete worker-management workflow with tools such as ``agent_create``, ``agent_update``, ``agent_run``, ``agent_status``, ``agent_list``, ``agent_wait``, ``agent_stop``, ``agent_remove``, ``workflow_status``, and ``workflow_finish``. Enabled plugin/provider tools, ``shared_context`` and ``query_index`` are also available when allowed by the current preset/runtime.

A custom profile has no implicit built-in main role prompt: if its system-prompt field is empty, PyGPT does not substitute Chat, Orchestrator or Swarm instructions. Use **From defaults** when you want a starting template; for custom profiles it loads the complete built-in Orchestrator prompt. The editor also includes an in-place Help reference for the agent workflow tools and runtime context blocks such as ``%workdir%``, ``<runtime_capabilities>``, ``<runtime_environment>``, ``<additional_system_prompt>``, ``<additional_project_rules>``, ``<additional_context>``, ``<rag_access>``, ``<workflow_language>``, and ``<worker_identity>``.
