...
def test_completion(mock_window):
    ...
    gpt.window.app.context.get_prompt_items.return_value = []
    gpt.window.app.context.get_model_ctx.return_value = 2048
    ...
    gpt.window.app.context.add_item = MagicMock()
...
def test_build_chat_messages(mock_window):
    ...
    gpt.window.app.context.get_prompt_items.return_value = items
    gpt.window.app.context.get_model_ctx.return_value = 2048
...
def test_prepare_ctx_name(mock_window):
    ...
    gpt.window.config.get.side_effect = mock_get
    gpt.window.config.get_model_ctx = MagicMock(return_value=2048)
    gpt.window.app.context.get_prompt_items = MagicMock(return_value=[])
    response = gpt.prepare_ctx_name(ContextItem())
    assert response == 'test_response'
