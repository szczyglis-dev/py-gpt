...
def fake_window(fake_client):
    window = MagicMock()
    window.core.gpt.get_client.return_value = fake_client
    return window
...
def test_get_client(assistants_obj, fake_window):
    client = MagicMock()
    fake_window.core.gpt.get_client.return_value = client
...
def test_create(assistants_obj, fake_window):
    ...
    fake_window.core.gpt.get_client.return_value.beta.assistants.create.return_value = fake_result
    result = assistants_obj.create(assistant)
    fake_window.core.gpt.get_client.return_value.beta.assistants.create.assert_called_once_with(
        instructions="inst",
        description="desc",
        name="name",
        tools=[],
...
def test_update(assistants_obj, fake_window):
    ...
    fake_window.core.gpt.get_client.return_value.beta.assistants.update.return_value = fake_result
    result = assistants_obj.update(assistant)
    fake_window.core.gpt.get_client.return_value.beta.assistants.update.assert_called_once_with(
        "old_id",
        instructions="inst",
        description="desc",
        name="name",
...
def test_delete(assistants_obj, fake_window):
    ...
    fake_window.core.gpt.get_client.return_value.beta.assistants.delete.return_value = fake_response
    result = assistants_obj.delete("some_id")
    fake_window.core.gpt.get_client.return_value.beta.assistants.delete.assert_called_once_with("some_id")
    assert result == "deleted_id"

def test_thread_create(assistants_obj, fake_window):
    fake_thread = MagicMock()
    fake_thread.id = "thread_id"
    fake_window.core.gpt.get_client.return_value.beta.threads.create.return_value = fake_thread
    result = assistants_obj.thread_create()
    fake_window.core.gpt.get_client.return_value.beta.threads.create.assert_called_once()
    assert result == "thread_id"

def test_thread_delete(assistants_obj, fake_window):
    fake_response = MagicMock()
    fake_response.id = "thread_deleted_id"
    fake_window.core.gpt.get_client.return_value.beta.threads.delete.return_value = fake_response
    result = assistants_obj.thread_delete("thread1")
    fake_window.core.gpt.get_client.return_value.beta.threads.delete.assert_called_once_with("thread1")
    assert result == "thread_deleted_id"

def test_msg_list(assistants_obj, fake_window):
    fake_list = MagicMock()
    fake_list.data = ["msg1", "msg2"]
    fake_window.core.gpt.get_client.return_value.beta.threads.messages.list.return_value = fake_list
    result = assistants_obj.msg_list("thread1")
    fake_window.core.gpt.get_client.return_value.beta.threads.messages.list.assert_called_once_with("thread1")
    assert result == ["msg1", "msg2"]

def test_msg_send_without_files(assistants_obj, fake_window):
    fake_message = MagicMock()
    fake_window.core.gpt.get_client.return_value.beta.threads.messages.create.return_value = fake_message
    result = assistants_obj.msg_send("thread1", "hello", [])
    fake_window.core.gpt.get_client.return_value.beta.threads.messages.create.assert_called_once_with(
        "thread1",
        role="user",
        content="hello"
    )
...
def test_run_create(assistants_obj, fake_window):
    fake_run = MagicMock()
    fake_run.id = "run_id"
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.create.return_value = fake_run
    result = assistants_obj.run_create("thread1", "assistant1", model="modelX", instructions="inst")
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.create.assert_called_once_with(
...
def test_run_create_stream(assistants_obj, fake_window):
    ...
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.stream.return_value = fake_cm
    signals = MagicMock()
    ctx = MagicMock()
    result = assistants_obj.run_create_stream(signals, ctx, "thread1", "assistant1", model="modelX", instructions="inst")
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.stream.assert_called_once()
...
def test_run_status(assistants_obj, fake_window):
    ...
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.retrieve.return_value = fake_run
    ctx = MagicMock()
    ctx.thread = "thread1"
    ctx.run_id = "run1"
    status = assistants_obj.run_status(ctx)
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.retrieve.assert_called_once_with(
...
def test_run_stop(assistants_obj, fake_window):
    ...
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.cancel.return_value = fake_run
    ctx = MagicMock()
    ctx.thread = "thread1"
    ctx.run_id = "run1"
    status = assistants_obj.run_stop(ctx)
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.cancel.assert_called_once_with(
        thread_id="thread1", run_id="run1"
    )
...
def test_run_submit_tool(assistants_obj, fake_window):
    fake_response = MagicMock()
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.submit_tool_outputs.return_value = fake_response
    ctx = MagicMock()
    ctx.thread = "thread1"
    ctx.run_id = "run1"
    ...
    fake_window.core.gpt.get_client.return_value.beta.threads.runs.submit_tool_outputs.assert_called_once_with(
        thread_id="thread1",
        run_id="run1",
