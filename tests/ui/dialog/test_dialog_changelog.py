...
def test_setup_loads_changelog_and_builds_info_dialog():
    ...
    with patch("builtins.open", mock_open(read_data="Changes")) as opened, \
         ...
         patch("pygpt_net.ui.dialog.changelog.QPushButton", return_value=close_btn) as qbutton, \
    ...
    close_btn.setCursor.assert_called_once()
    close_btn.setSizePolicy.assert_called_once()
    close_btn.clicked.connect.assert_called_once()
    layout.addWidget.assert_any_call(updated_label)
    layout.addWidget.assert_any_call(textarea)
    layout.addWidget.assert_any_call(close_btn)
...
def test_setup_close_button_closes_changelog_dialog():
    window = _window(app_updated=False)
    close_btn = MagicMock()
    dialog = MagicMock()
    connected = {}

    close_btn.clicked.connect.side_effect = lambda callback: connected.setdefault("callback", callback)
    ...
    with patch("builtins.open", mock_open(read_data="Changes")), \
         ...
         patch("pygpt_net.ui.dialog.changelog.QPushButton", return_value=close_btn), \
...
def test_setup_hides_updated_label_when_not_after_update():
    ...
    with patch("builtins.open", mock_open(read_data="Changes")), \
         ...
         patch("pygpt_net.ui.dialog.changelog.QPushButton", return_value=MagicMock()), \
         patch("pygpt_net.ui.dialog.changelog.QVBoxLayout", return_value=MagicMock()), \
