...
def test_download(mock_window):
    ...
    data = MagicMock()
    data.filename = 'test'
    mock_window.core.openai.store.get_file = MagicMock(return_value=data)
    ...
    with patch('os.path.join') as os_path_join, \
        ...
        mock_window.core.openai.store.download.assert_called_once_with(file_id='file_id', path='path')
        assert result == 'path'
