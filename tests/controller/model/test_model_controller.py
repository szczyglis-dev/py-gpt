...
class DummyModelItem:
    def __init__(self, name, provider, is_hidden=False):
        self.name = name
...
def test_next(dummy_window):
    ...
    dummy_window.core.models.get_by_mode = MagicMock(return_value={
        "current_model": DummyModelItem("Current", "prov"),
        "next_model": DummyModelItem("Next", "prov"),
    })
    model_inst.next()
    model_inst.select.assert_called_once_with("next_model")

def test_next_skips_hidden(dummy_window):
    dummy_window.controller.chat.input.generating = False
    model_inst = Model(dummy_window)
    model_inst.select = MagicMock()
    dummy_window.core.config.data["model"] = "current_model"
    dummy_window.core.models.get_by_mode = MagicMock(return_value={
        "current_model": DummyModelItem("Current", "prov"),
        "hidden_model": DummyModelItem("Hidden", "prov", is_hidden=True),
        "next_model": DummyModelItem("Next", "prov"),
    })
    model_inst.next()
    model_inst.select.assert_called_once_with("next_model")
...
def test_prev(dummy_window):
    ...
    dummy_window.core.config.data["model"] = "current_model"
    dummy_window.core.models.get_by_mode = MagicMock(return_value={
        "prev_model": DummyModelItem("Previous", "prov"),
        "current_model": DummyModelItem("Current", "prov"),
    })
    model_inst.prev()
    model_inst.select.assert_called_once_with("prev_model")

def test_prev_skips_hidden(dummy_window):
    dummy_window.controller.chat.input.generating = False
    model_inst = Model(dummy_window)
    model_inst.select = MagicMock()
    dummy_window.core.config.data["model"] = "current_model"
...
def test_select_default_default(dummy_window):
    ...
    dummy_window.core.models.get_by_mode = MagicMock(return_value={
        "default_model": DummyModelItem("Default", "prov"),
    })
    model_inst.select_default()
    assert dummy_window.core.config.data["model"] == "default_model"
    assert dummy_window.core.config.data["current_model"]["test_mode"] == "default_model"
...
def test_init_list(dummy_window):
    model_inst = Model(dummy_window)
    # Setup visible model items with different providers
...
def test_init_list_skips_hidden(dummy_window):
    model_inst = Model(dummy_window)
    visible = DummyModelItem("Visible", "prov1")
    hidden = DummyModelItem("Hidden", "prov1", is_hidden=True)
    dummy_window.core.models.get_by_mode = MagicMock(return_value={
        "visible": visible,
        "hidden": hidden,
    })
    dummy_window.core.llm.get_choices = MagicMock(return_value={"prov1": "Provider 1"})
