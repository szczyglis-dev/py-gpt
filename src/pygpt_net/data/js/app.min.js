...
class Loading{constructor(dom){this.dom=dom;this._showFrame=null;this._hideTimer=null;this._hideHandler=null;this._transitionToken=0;}
_cancelPending(el){this._transitionToken+=1;if(this._showFrame!==null){try{cancelAnimationFrame(this._showFrame);}catch(_){}this._showFrame=null;}if(this._hideTimer!==null){try{clearTimeout(this._hideTimer);}catch(_){}this._hideTimer=null;}if(el&&this._hideHandler){try{el.removeEventListener('transitionend',this._hideHandler);}catch(_){}}this._hideHandler=null;}
show(){if(typeof window.hideTips==='function'){window.hideTips();}const el=this.dom.get('_loader_');if(!el)return;if(el.classList.contains('visible'))return;this._cancelPending(el);const token=this._transitionToken;el.classList.remove('hidden','visible');el.classList.add('reserved');void el.offsetWidth;this._showFrame=requestAnimationFrame(()=>{this._showFrame=null;if(token!==this._transitionToken)return;el.classList.remove('reserved');el.classList.add('visible');});}
hide(reserveSpace=false){const el=this.dom.get('_loader_');if(!el)return;const wasVisible=el.classList.contains('visible');const wasReserved=el.classList.contains('reserved');this._cancelPending(el);const token=this._transitionToken;if(el.classList.contains('hidden'))return;el.classList.remove('visible','hidden');el.classList.add('reserved');if(reserveSpace)return;const finish=()=>{if(token!==this._transitionToken)return;if(this._hideTimer!==null){try{clearTimeout(this._hideTimer);}catch(_){}this._hideTimer=null;}if(this._hideHandler){try{el.removeEventListener('transitionend',this._hideHandler);}catch(_){}this._hideHandler=null;}if(!el.classList.contains('visible')){el.classList.remove('reserved');el.classList.add('hidden');}};if(!wasVisible&&wasReserved){finish();return;}this._hideHandler=(event)=>{if(event.target!==el||event.propertyName!=='opacity')return;finish();};el.addEventListener('transitionend',this._hideHandler);this._hideTimer=setTimeout(finish,400);}}
...
restoreCollapsedCode(root){const scope=root||document;const wrappers=scope.querySelectorAll('.code-wrapper');wrappers.forEach((wrapper)=>{const index=wrapper.getAttribute('data-index');const localeCollapse=wrapper.getAttribute('data-locale-collapse');const localeExpand=wrapper.getAttribute('data-locale-expand');const source=wrapper.querySelector('code');const isCollapsed=(window.__collapsed_idx||[]).includes(index);if(!source)return;try{source.style.removeProperty('display');}catch(_){}wrapper.classList.toggle('code-collapsed',isCollapsed);wrapper.setAttribute('aria-expanded',isCollapsed?'false':'true');const btn=wrapper.querySelector('.code-header-collapse');if(btn){const span=btn.querySelector('span');if(span)span.textContent=isCollapsed?localeExpand:localeCollapse;btn.setAttribute('title',(isCollapsed?localeExpand:localeCollapse)||(isCollapsed?'Expand':'Collapse'));btn.setAttribute('aria-expanded',isCollapsed?'false':'true');}});}};
...
/* data/js/app/user.js */
class UserCollapseManager{constructor(cfg){this.cfg=cfg||{};this.threshold=Utils.g('USER_MSG_COLLAPSE_HEIGHT_PX',350);this._processed=new Set();this._remeasureTimer=null;this._fontsReadyScheduled=false;}
...
_scheduleStableRemeasure(){if(this._remeasureTimer!==null){try{clearTimeout(this._remeasureTimer);}catch(_){}}const remeasure=()=>{this._remeasureTimer=null;this._afterLayout(()=>this._afterLayout(()=>this.remeasureAll()));};this._remeasureTimer=setTimeout(remeasure,80);if(!this._fontsReadyScheduled){this._fontsReadyScheduled=true;try{if(document.fonts&&document.fonts.ready&&typeof document.fonts.ready.then==='function'){document.fonts.ready.then(()=>{this._afterLayout(()=>this.remeasureAll());}).catch(()=>{});}}catch(_){}}}
apply(root){const scope=root||document;let list;if(scope.nodeType===1)list=scope.querySelectorAll('.msg-box.msg-user .msg');else list=document.querySelectorAll('.msg-box.msg-user .msg');if(!list||!list.length)return;for(let i=0;i<list.length;i++){const msg=list[i];const st=this._ensureStructure(msg);if(!st)continue;this._update(msg,st.content,st.toggle);}this._scheduleStableRemeasure();}
...
static escapeHtml(s){const d=Utils._escDiv||(Utils._escDiv=document.createElement('div'));d.textContent=String(s??'');return d.innerHTML;}static escapeHtmlAttr(s){return Utils.escapeHtml(s).replace(/\"/g,'&quot;').replace(/'/g,'&#39;');}
static decodeMentionValue(s){return String(s??'').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&quot;/g,'"').replace(/&#x27;|&#39;/g,"'").replace(/&amp;/g,'&');}
static renderMentionText(s){const raw=String(s??'');const re=/<(attachment|file_context)>([\s\S]*?)<\/\1>|<conversation\b([^>]*)>([\s\S]*?)<\/conversation>/gi;let out='';let last=0;let match;while((match=re.exec(raw))!==null){out+=Utils.escapeHtml(raw.slice(last,match.index));if(match[1]){const kind=String(match[1]||'').toLowerCase();const value=Utils.decodeMentionValue(match[2]||'');let label=value;if(kind==='file_context'){let normalized=value.replace(/\\/g,'/');const lower=normalized.toLowerCase();const prefix='%workdir%/data/';if(lower.startsWith(prefix))normalized=normalized.slice(prefix.length);else if(lower==='%workdir%/data')normalized='data/';else if(lower.startsWith('data/'))normalized=normalized.slice(5);label=normalized||value;}const cls=(kind==='attachment')?'mention-attachment':'mention-file-context';out+=`<span class="mention-anchor ${cls}" title="${Utils.escapeHtmlAttr(value)}">@${Utils.escapeHtml(label)}</span>`;}else{const attrs=String(match[3]||'');const idMatch=attrs.match(/\bid\s*=\s*(["'])(.*?)\1/i);const titleMatch=attrs.match(/\btitle\s*=\s*(["'])(.*?)\1/i);const id=Utils.decodeMentionValue(idMatch?idMatch[2]:'');const title=Utils.decodeMentionValue(titleMatch?titleMatch[2]:'');const label=title||id;if(label){out+=`<span class="mention-anchor mention-conversation" title="#${Utils.escapeHtmlAttr(id)}">@${Utils.escapeHtml(label)}</span>`;}else{out+=Utils.escapeHtml(match[0]);}}last=re.lastIndex;}out+=Utils.escapeHtml(raw.slice(last));return out.replace(/\r?\n/g,'<br>');}
