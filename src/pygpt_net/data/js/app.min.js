...
_scheduleReasoningHide(msg,rootOverride=null){if(!this.reasoningEnabled||!this.reasoningHideAfterResponse||!this.reasoningVisible||this.reasoningThinking)return;if(this.reasoningHideDelayTimer||this.reasoningFadeOutTimer||this.reasoningFadeOutStartedAt>0)return;const getRoot=()=>rootOverride||this.getMsgSnapshotRoot(msg);const startFade=()=>{this.reasoningHideDelayTimer=0;if(this.reasoningThinking||!this.reasoningVisible)return;const duration=Math.max(0,Number(this.reasoningFadeDuration)||0);this.reasoningVisible=false;this.reasoningFadeInStartedAt=0;this.reasoningFadeOutStartedAt=(typeof performance!=='undefined'&&typeof performance.now==='function')?performance.now():Date.now();this._syncReasoningVisibility(getRoot());if(duration<=0){this.reasoningFadeOutStartedAt=0;this._syncReasoningVisibility(getRoot());return;}
this.reasoningFadeOutTimer=setTimeout(()=>{this.reasoningFadeOutTimer=0;if(this.reasoningThinking)return;this.reasoningFadeOutStartedAt=0;this._syncReasoningVisibility(getRoot());},duration+24);};const delay=Math.max(0,Number(this.reasoningFadeOutDelay)||0);if(delay<=0)startFade();else this.reasoningHideDelayTimer=setTimeout(startFade,delay);}
hideReasoningForToolCall(root=null){if(!this.reasoningEnabled||!this.reasoningHideAfterResponse||!this.reasoningVisible)return;this.reasoningThinking=false;const msg=this.getMsg(false,'');if(!msg&&!root)return;this._scheduleReasoningHide(msg,root);}
...
if(this.snapshotScheduled&&!this.raf.isScheduled('SE:snapshot'))this.snapshotScheduled=false;const msg=this.getMsg(true,name_header);if(!msg||!chunk)return;const s=String(chunk);const reasoningState=this._updateReasoningVisibilityFromChunk(s);const reasoningOpened=reasoningState.changed&&this.reasoningThinking;if(reasoningState.hasResponseText&&!this.reasoningThinking){this._scheduleReasoningHide(msg);}
if(/[<>]/.test(s)){this._d('apply.chunk',{len:s.length,nl:Utils.countNewlines(s),head:s.slice(0,120),tail:s.slice(-120)});}
if(!alreadyBuffered)this._appendChunk(s);if(reasoningOpened&&this.reasoningEnabled&&!this.fenceOpen&&!this.codeStream.open){try{this.renderSnapshot(msg);try{this.raf.cancel('SE:snapshot');}catch(_){}this.snapshotScheduled=false;}catch(_){}}const change=this.updateFenceHeuristic(s);const nlCount=Utils.countNewlines(s);const chunkHasNL=nlCount>0;if(!change.opened&&!this.fenceOpen){this._maybeEagerSnapshotForCustomOpeners(msg,s);}
