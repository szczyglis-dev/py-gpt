/* app.min.js — generated on 2025-09-22 02:38:36 by bin/minify_js.py using rjsmin */
...
_flashCopied(btn,wrapper){if(!btn)return;const span=btn.querySelector('span');if(!span){try{if(btn.__copyTimer){clearTimeout(btn.__copyTimer);btn.__copyTimer=0;}}catch(_){}
btn.classList.add('copied');btn.__copyTimer=setTimeout(()=>{try{btn.classList.remove('copied');}catch(_){}
btn.__copyTimer=0;},1200);return;}
const L_COPY=(wrapper&&wrapper.getAttribute('data-locale-copy'))||'Copy';const L_COPIED=(wrapper&&wrapper.getAttribute('data-locale-copied'))||'Copied';try{if(btn.__copyTimer){clearTimeout(btn.__copyTimer);btn.__copyTimer=0;}}catch(_){}
span.textContent=L_COPIED;btn.classList.add('copied');btn.__copyTimer=setTimeout(()=>{try{span.textContent=L_COPY;btn.classList.remove('copied');}catch(_){}
btn.__copyTimer=0;},1200);}
