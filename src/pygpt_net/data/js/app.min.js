...
_renderCollapsibleExtraRows(rows){if(!Array.isArray(rows)||!rows.length)return'';let limit=5;try{const configured=Number((typeof window!=='undefined')?window.EXTRA_ITEMS_VISIBLE_LIMIT:limit);if(Number.isFinite(configured))limit=Math.floor(configured);}catch(_){}if(limit<=0||rows.length<=limit){return`<div class="extra-items-list">${rows.join("<br/>")}</div>`;}const visible=rows.slice(0,limit).join("<br/>");const hidden=rows.slice(limit).join("<br/>");const remaining=rows.length-limit;const labelTpl=(typeof window!=='undefined'&&window.LOCALE_MORE_ITEMS)?String(window.LOCALE_MORE_ITEMS):'+ {count} more items';const label=labelTpl.split('{count}').join(String(remaining));const expandTitle=(typeof window!=='undefined'&&window.LOCALE_EXPAND)?String(window.LOCALE_EXPAND):'Expand';const expIcon=(typeof window!=='undefined'&&window.ICON_EXPAND)?String(window.ICON_EXPAND):'';const arrow=expIcon?`<img src="${this._esc(expIcon)}" class="extra-items-toggle-arrow" alt="">`:'';return(`<div class="extra-items-list">`+`<div class="extra-items-visible">${visible}</div>`+`<div class="extra-items-hidden" style="display:none">${hidden}</div>`+`<button type="button" class="extra-items-toggle" onclick="toggleExtraItems(this);" `+`title="${this._escapeHtml(expandTitle)}" aria-expanded="false">`+`<span class="extra-items-toggle-label">${this._escapeHtml(label)}</span>${arrow}`+`</button>`+`</div>`);}
...
const files=block.files||{};const kF=Object.keys(files);if(kF.length){const rows=[];kF.forEach((k)=>{const it=files[k];if(!it)return;const url=this._esc(it.url);const path=this._esc(it.path);const icon=(typeof window!=='undefined'&&window.ICON_ATTACHMENTS)?`<img src="${window.ICON_ATTACHMENTS}" class="extra-src-icon">`:'';rows.push(`${icon} <a href="${url}">${path}</a> <b> [${k}] </b>`);});if(rows.length)parts.push(this._renderCollapsibleExtraRows(rows));}
const urls=block.urls||{};const kU=Object.keys(urls);if(kU.length){const rows=[];kU.forEach((k)=>{const it=urls[k];if(!it)return;const url=this._esc(it.url);const icon=(typeof window!=='undefined'&&window.ICON_URL)?`<img src="${window.ICON_URL}" class="extra-src-icon">`:'';rows.push(`${icon}<a href="${url}" title="${url}">${url}</a> <small> [${k}] </small>`);});if(rows.length)parts.push(this._renderCollapsibleExtraRows(rows));}
...
_renderToolOutputWrapper(block){const extra=block.extra||{};const toolCalls=Array.isArray(extra.tool_calls)?extra.tool_calls.filter(Boolean):[];const hasToolCalls=toolCalls.length>0;const legacyToolOutput=(extra.tool_output!=null)?String(extra.tool_output):'';const resultHtml=(extra.tool_result!=null)?this._escapeHtml(String(extra.tool_result)):legacyToolOutput;const wrapperDisplay=(extra.tool_output_visible===true||hasToolCalls)?'':'display:none';const toggleTitle=(typeof trans!=='undefined'&&trans)?trans('action.cmd.expand'):'Expand';const expIcon=(typeof window!=='undefined'&&window.ICON_EXPAND)?window.ICON_EXPAND:'';const toolIcon=(typeof window!=='undefined'&&window.ICON_TOOL)?window.ICON_TOOL:'';const toolLabel=(typeof window!=='undefined'&&window.LOCALE_TOOL)?window.LOCALE_TOOL:'Tool';const requestLabel=(typeof window!=='undefined'&&window.LOCALE_TOOL_REQUEST)?window.LOCALE_TOOL_REQUEST:'Request';const responseLabel=(typeof window!=='undefined'&&window.LOCALE_TOOL_RESPONSE)?window.LOCALE_TOOL_RESPONSE:'Response';let titleHtml='';let contentHtml=legacyToolOutput;if(hasToolCalls){const names=toolCalls.map((call)=>this._escapeHtml(String(call.name||'tool')));const requests=toolCalls.map((call)=>this._escapeHtml(String(call.request||''))).join('\n\n');const iconHtml=toolIcon?`<img src='${this._esc(toolIcon)}' class='tool-output-icon' alt=''>`:'';const arrowHtml=`<img src='${this._esc(expIcon)}' class='tool-output-arrow' width='25' height='25' alt=''>`;titleHtml=`<button type='button' class='tool-output-toggle' onclick='toggleToolOutput(${this._esc(block.id)});' `+`title='${this._escapeHtml(toggleTitle)}' aria-expanded='false'>`+`${iconHtml}<span class='tool-output-label'><b>${this._escapeHtml(toolLabel)}:</b>&nbsp;</span>`+`<span class='tool-output-name'>${names.join(', ')}</span>${arrowHtml}`+`</button>`;contentHtml=`<div class='tool-output-section'>`+`<b>${this._escapeHtml(requestLabel)}:</b>`+`<div class='tool-output-data tool-output-request-data'>${requests}</div>`+`</div>`+`<div class='tool-output-section'>`+`<b>${this._escapeHtml(responseLabel)}:</b>`+`<div class='tool-output-data tool-output-result-data'>${resultHtml}</div>`+`</div>`;}const legacyToggleHtml=hasToolCalls?'':`<span class='toggle-cmd-output' onclick='toggleToolOutput(${this._esc(block.id)});' `+`title='${this._escapeHtml(toggleTitle)}' role='button'>`+`<img src='${this._esc(expIcon)}' width='25' height='25' valign='middle'>`+`</span>`;return(`<div class='tool-output' style='${wrapperDisplay}'>`+`${titleHtml}${legacyToggleHtml}`+`<div class='content' style='display:none' data-trusted='1'>${contentHtml}</div>`+`</div>`);}
_renderBot(block){const id=block.id;const out=block.output||{};const msgId=`msg-bot-${id}`;const personalize=!!(block&&block.extra&&block.extra.personalize===true);const nameHeader=personalize?this._nameHeader('bot',out.name||'',out.avatar_img||null):'';const mdText=this._escapeHtml(out.text||'');const mdBlock=mdText?`<div class='md-block' md-block-markdown='1'>${mdText}</div>`:'';const toolWrap=this._renderToolOutputWrapper(block);const extras=this._renderExtras(block);const actions=(block.extra&&block.extra.footer_icons)?this._renderActions(block):'';const debug=(block.extra&&block.extra.debug_html)?String(block.extra.debug_html):'';return(`<div class='msg-box msg-bot' id='${msgId}'>`+`${nameHeader}`+`<div class='msg'>`+`${mdBlock}`+`<div class='msg-tool-extra'></div>`+`${toolWrap}`+`<div class='msg-extra'>${extras}</div>`+`${actions}${debug}`+`</div>`+`</div>`);}
renderNode(block){const parts=[];if(block&&block.input&&block.input.text)parts.push(this._renderUser(block));if(block&&block.output){const extra=block.extra||{};const hasToolCalls=Array.isArray(extra.tool_calls)&&extra.tool_calls.length>0;if(block.output.text||hasToolCalls||extra.tool_output_visible===true){parts.push(this._renderBot(block));}}return parts.join('');}
...
append(content){this.hideLoader();this.enable();const els=document.querySelectorAll('.tool-output');if(els.length){const contentEl=els[els.length-1].querySelector('.content');if(!contentEl)return;const resultEl=contentEl.querySelector('.tool-output-result-data');if(resultEl){resultEl.insertAdjacentText('beforeend',content==null?'':String(content));}else{contentEl.insertAdjacentHTML('beforeend',content==null?'':String(content));}}}
update(content){this.hideLoader();this.enable();const els=document.querySelectorAll('.tool-output');if(els.length){const contentEl=els[els.length-1].querySelector('.content');if(!contentEl)return;const resultEl=contentEl.querySelector('.tool-output-result-data');if(resultEl){resultEl.textContent=content==null?'':String(content);}else{contentEl.innerHTML=content==null?'':String(content);}}}
clear(){this.hideLoader();this.enable();const els=document.querySelectorAll('.tool-output');if(els.length){const contentEl=els[els.length-1].querySelector('.content');if(!contentEl)return;const resultEl=contentEl.querySelector('.tool-output-result-data');if(resultEl)resultEl.replaceChildren();else contentEl.replaceChildren();}}
toggle(id){const el=document.getElementById('msg-bot-'+id);if(!el)return;const outputEl=el.querySelector('.tool-output');if(!outputEl)return;const contentEl=outputEl.querySelector('.content');if(!contentEl)return;const expanded=contentEl.style.display==='none';contentEl.style.display=expanded?'block':'none';const headerEl=outputEl.querySelector('.tool-output-toggle');if(headerEl)headerEl.setAttribute('aria-expanded',expanded?'true':'false');const arrowEl=outputEl.querySelector('.tool-output-arrow')||outputEl.querySelector('.toggle-cmd-output img');if(arrowEl)arrowEl.classList.toggle('toggle-expanded',expanded);}};
