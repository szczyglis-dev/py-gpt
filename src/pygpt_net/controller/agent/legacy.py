...
from pygpt_net.plugin.agent.prompts import (
    ...
    JUDGE_PROMPT as DEFAULT_JUDGE_PROMPT,
...
Output only concrete new/corrective material or a necessary consolidated replacement. Never repeat or paraphrase earlier text merely to consume an iteration. Always answer in the language of the original user request unless the user explicitly requested another language."""
    ...
    JUDGE_ALWAYS_CONTINUE_NOTE = """# ALWAYS CONTINUE
The autonomous run is configured to continue even when the current answer appears complete. You MUST produce a useful next-pass instruction. If the obvious work is exhausted, choose a fresh, non-redundant in-scope verification, challenge an assumption, test an edge case or failure mode, derive the result another way, tighten correctness/precision/robustness, or examine a relevant second-order implication. Never tell the assistant to finish and never respond that nothing remains to do."""
...
"""
    ...
    def __init__(self, window=None):
        ...
        self.run_dynamic_continue = None
        self.run_dynamic_messages = None
    ...
    def get_dynamic_continue(self) -> bool:
        """Return whether continuation instructions should be generated by a judge call."""
        if self.run_active and self.run_dynamic_continue is not None:
            return bool(self.run_dynamic_continue)
        if self.window.core.config.get('mode') == MODE_AGENT:
            value = self.window.core.config.get('agent.continue.dynamic')
            return True if value is None else bool(value)
        if self.is_inline() and self.window.controller.plugins.is_enabled("agent"):
            try:
                value = self.window.core.plugins.get_option("agent", "dynamic_continue")
                return True if value is None else bool(value)
            except Exception:
                return True
        return True
    ...
    @staticmethod
    def _normalize_dynamic_messages(value) -> int:
        """Normalize the number of assistant responses included in judge context."""
        try:
            return max(0, int(value))
        except (TypeError, ValueError):
            return 3

    def get_dynamic_messages(self) -> int:
        """Return max assistant responses sent to the judge; 0 means the whole current run."""
        if self.run_active and self.run_dynamic_messages is not None:
            return self._normalize_dynamic_messages(self.run_dynamic_messages)
        if self.window.core.config.get('mode') == MODE_AGENT:
            return self._normalize_dynamic_messages(
                self.window.core.config.get('agent.continue.dynamic.messages')
            )
        if self.is_inline() and self.window.controller.plugins.is_enabled("agent"):
            try:
                return self._normalize_dynamic_messages(
                    self.window.core.plugins.get_option("agent", "dynamic_continue_messages")
                )
            except Exception:
                return 3
        return 3
    ...
    def get_judge_prompt(self, always_continue: Optional[bool] = None) -> str:
        """Return the configured hidden evaluator instruction."""
        value = str(self.window.core.prompt.get("agent.continue.judge") or "").strip()
        if not value:
            value = self.JUDGE_PROMPT
        if always_continue is None:
            always_continue = self.get_always_continue()
        if always_continue:
            value = value.rstrip() + "\n\n" + self.JUDGE_ALWAYS_CONTINUE_NOTE
        return value

    @staticmethod
    def _assistant_responses(ctx: CtxItem) -> List[str]:
        """Return visible assistant responses grouped by autonomous provider pass."""
        if ctx is None:
            return []

        parts = list(getattr(ctx, "parts", None) or [])
        if not parts:
            output = str(ctx.final_output or "").strip()
        ...
        for part in parts:
            ...
            starts_pass = isinstance(extra, dict) and bool(extra.get("agent_continue"))
            if starts_pass and current:
                response = "\n\n".join(current).strip()
                if response:
            ...
            if output is not None and str(output).strip():
                current.append(str(output).strip())
    ...
    def _judge_conversation(self, ctx: CtxItem) -> str:
        """Build the configured tail of the current user turn for the hidden judge."""
        root = self.run_root or ctx
        human = str(getattr(root, "input", None) or "")
        responses = self._assistant_responses(root)
        limit = self.get_dynamic_messages()
        total = len(responses)
        if limit > 0 and total > limit:
            responses = responses[-limit:]

        first_pass = total - len(responses) + 1
        chunks = ["Human:\n{}".format(human)]
        for offset, response in enumerate(responses):
            pass_no = first_pass + offset
            chunks.append("Assistant (pass {}):\n{}".format(pass_no, response))
        return "\n\n".join(chunks)

    def _run_dynamic_judge(
            self,
            run_id: int,
            model_key: str,
            conversation: str,
        ...
        """Execute the hidden judge request in a worker and emit its instruction."""
        output = ""
        error = None
        try:
            core = self.window.core
            model = core.models.get(model_key)
            if model is None:
                raise RuntimeError("Autonomous judge model is unavailable: {}".format(model_key))

            judge_ctx = CtxItem(mode=MODE_CHAT)
            judge_ctx.model = model_key
            ...
            output = str(core.bridge.call(context, extra={"disable_tools": True}) or "").strip()
    ...
    def _start_dynamic_judge(self, ctx: CtxItem, iterations: int) -> bool:
        """Start a hidden judge pass that will enqueue the next autonomous iteration."""
        if not self.get_dynamic_continue() or self.stop or self.finished:
            return False
        if not self.is_ctx_current_run(ctx):
            return False

        root = self.run_root or ctx
        model_key = str(getattr(root, "model", None) or getattr(ctx, "model", None) or "")
        if not model_key:
            return False

        run_id = self.run_id
        worker = Worker(self._run_dynamic_judge)
        worker.signals.finished.connect(self._on_dynamic_judge_finished)
        worker.kwargs.update({
            "run_id": run_id,
            "model_key": model_key,
            "conversation": self._judge_conversation(ctx),
            "judge_prompt": self.get_judge_prompt(always_continue=self.get_always_continue()),
            "finished_signal": worker.signals.finished,
        })
        self.judge_workers[run_id] = {
            "worker": worker,
            "ctx": root,
            "iterations": iterations,
        }

        # CTX_END will make the renderer idle immediately after this callback.
        # Keep user input locked while the invisible evaluator prepares the next
        # iteration so a second manual request cannot interleave with this run.
        self.window.controller.chat.common.lock_input()
        self.window.threadpool.start(worker)
        return True

    def _on_dynamic_judge_finished(self, data):
        """Queue the next provider pass after a hidden judge worker finishes."""
        if not isinstance(data, dict):
            return
        run_id = data.get("run_id")
        state = self.judge_workers.pop(run_id, None)
        if state is None:
            return
        if run_id != self.run_id or not self.run_active or self.stop or self.finished:
            return
        ...
        if not prompt:
            ...
            if error:
                self.window.core.debug.info(
                    "[agent] Dynamic judge failed; falling back to static continuation prompt: {}".format(error)
                )
            else:
                self.window.core.debug.info(
                    "[agent] Dynamic judge returned an empty instruction; falling back to static continuation prompt."
                )
    ...
    def on_user_send(self, text: str):
        ...
        auto_stop = self.get_auto_stop()
        always_continue = self.get_always_continue()
        dynamic_continue = self.get_dynamic_continue()
        dynamic_messages = self.get_dynamic_messages()
        ...
        self.run_dynamic_continue = bool(dynamic_continue)
        self.run_dynamic_messages = self._normalize_dynamic_messages(dynamic_messages)
    ...
    def on_ctx_end(
        ...
        if self._start_dynamic_judge(ctx, limit):
            return
