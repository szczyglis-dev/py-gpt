    ...
    def send(
        ...
        if continuation_parent is not None:
            ...
            if agent_continue:
                ...
                if agent_judge:
                    part_extra["agent_judge"] = True
                    part_extra["judge_input"] = str(agent_judge_text or text)
                ...
                if agent_judge:
                    # Paint the judge instruction before the next provider stream.
                    # It lives only in partial metadata, so it is visible as a
                    # pseudo-input without becoming another user CtxItem/turn.
