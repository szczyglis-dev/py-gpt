...
class Query:
    ...
    def chat(self, ctx: CtxItem, idx: str = "base", model: str = "gpt-3.5-turbo", sys_prompt: str = None,
        ...
        # append context from DB to memory
        history = self.context.get_messages()
        memory = ChatMemoryBuffer.from_defaults(chat_history=history)
        self.input_tokens = self.window.core.tokens.from_llama_messages(query, history, model)
        ...
        ctx.input_tokens = self.input_tokens
        ctx.output_tokens = self.window.core.tokens.from_llama_messages(response, [], model)  # calc from response
